$(function(){
  ...
  // Module Search
    ...
    // Most Used
    $('#ShowMostUsedModules').click(function () {
        var $container = $('#Container');
        var $items = $container.find(".item");
        var avgUsage = $container.data("avgusage");

        var sorts = {
            usage: function ($item) { return $item.data("usage"); }
        };

        clearClass($items, "match");

        $.each($items, function (i, row) {
            var isMatch = $(row).data("usage") > avgUsage;
            if (isMatch) $(row).addClass("match");
        });

        $container.isotope({ getSortData: sorts, sortBy: 'usage', sortAscending: false });
        $container.isotope({ filter: '.match' });
